...
apply_theme()
st.header("🖼️ Publication Figures Suite")
...
fig_dirs = [
    ...
    Path("backtesting/results/_figures/png"),
]
...
if fig_dir is not None:
    ...
    if sel_fig_name:
        ...
        with open(sel_path, "rb") as f:
            st.download_button(
                label=f"⬇️ Download {sel_fig_name} (PNG)",
                data=f.read(),
                file_name=sel_fig_name,
                mime="image/png",
            )
else:
    st.warning("Figures not found on disk. Run `python main.py --figures` to render the visualization suite.")