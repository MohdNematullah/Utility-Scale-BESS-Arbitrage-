...
class TestExperimentsSuite:
    def test_28_scenario_generation(self, tmp_path: Path):
    ...
    def test_reproducibility_engine(self, tmp_path: Path):
    ...
    def test_runtime_benchmark_export(self, tmp_path: Path):
    ...
    def test_environment_snapshot(self, tmp_path: Path):
        snap = EnvironmentSnapshotter(output_dir=tmp_path / "env")
        j_p, t_p = snap.capture_snapshot()
        assert j_p.exists()
        assert t_p.exists()

    def test_final_report_builder(self, tmp_path: Path):
        runner = ExperimentSuiteRunner(output_dir=tmp_path / "exp")
        df = runner.run_all_scenarios()
        builder = FinalReportBuilder(output_dir=tmp_path / "report")
        j_p, c_p, x_p, m_p = builder.build_complete_package(
            df,
            {
    ...
    def test_artifact_manifest_generation(self, tmp_path: Path):
